...
/// Cubo AABB con centro y semiejes (half_extents)
pub struct Cube {
    pub center: Vec3,
    pub half: Vec3,
...
impl Hittable for Cube {
    fn intersect(&self, ray: &Ray, t_min: f32, t_max: f32) -> Option<HitRecord> {
        ...
        let mut t1 = (min_b.x - ray.origin.x) * inv_dx;
        let mut t2 = (max_b.x - ray.origin.x) * inv_dx;
        ...
        let mut t1 = (min_b.y - ray.origin.y) * inv_dy;
        let mut t2 = (max_b.y - ray.origin.y) * inv_dy;
        if t1 > t2 { std::mem::swap(&mut t1, &mut t2); }
        tmin = tmin.max(t1);
        tmax = tmax.min(t2);
        if tmax < tmin { return None; }
        ...
        let mut t1 = (min_b.z - ray.origin.z) * inv_dz;
        let mut t2 = (max_b.z - ray.origin.z) * inv_dz;
        if t1 > t2 { std::mem::swap(&mut t1, &mut t2); }
        tmin = tmin.max(t1);
        tmax = tmax.min(t2);
        if tmax < tmin { return None; }

        // El "hit" válido más cercano
        let t_hit = if tmin > t_min { tmin } else { tmax };
        if t_hit < t_min || t_hit > t_max { return None; }

        let p = ray.at(t_hit);

        // Normal por cara: detectamos cuál cara tocamos comparando con los límites
